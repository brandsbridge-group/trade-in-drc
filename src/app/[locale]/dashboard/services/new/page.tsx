...
export default function NewServicePage() {
  ...
  if (!user) {
    return (
      <div className="flex items-center justify-center min-h-[200px]">
        <p className="text-sm text-slate-500">{t("services.signInToAdd")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl space-y-4">
      ...
      <PageHeader title={t("services.addService")} />
      ...
      {loadingCompanies ? (
        ...
        <div className="bg-card border border-slate-200 rounded-2xl p-6 text-center">
          ...
          <p className="text-sm font-medium mb-1">{t("services.noCompany")}</p>
          <p className="text-xs text-muted-foreground mb-4">{t("services.noCompanyBody")}</p>
          <Button size="sm" asChild variant="outline">
            <Link href="/dashboard/companies">{t("services.registerCompany")}</Link>
          </Button>
        </div>
      ) : (
        <>
          {companies.length > 1 && (
            <div className="flex items-center gap-2">
              <span className="text-xs text-muted-foreground">{t("services.companyLabel")}</span>
              <Select value={selectedCompanyId ?? ""} onValueChange={setActiveCompany}>
                <SelectTrigger className="h-8 text-sm w-56">
                  <SelectValue placeholder={t("services.selectCompany")} />
                </SelectTrigger>
                <SelectContent>
          ...
          <AnimatedField show={!!selectedCompanyId}>
            <div className="bg-card border border-slate-200 rounded-2xl p-4">
              {selectedCompanyId && <ServiceForm companyId={selectedCompanyId} />}
            </div>
          </AnimatedField>
        </>
      )}
